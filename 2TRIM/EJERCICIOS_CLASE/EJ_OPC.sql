...
INSERT INTO producto VALUES ('OR-99','Mimosa DEALBATA Gaulois Astier  ','Ornamentales','200-225','Viveros EL OASIS',
'Acacia dealbata. Nombre común o vulgar: Mimosa fina, Mimosa, Mimosa común, Mimosa plateada, Aromo francés. Familia: Mimosaceae.
Origen: Australia, Sureste, (N. G. del Sur y Victoria). Arbol de follaje persistente muy usado en parques por su atractiva 
floración amarilla hacia fines del invierno. Altura: de 3 a 10 metros generalmente. Crecimiento rápido. Follaje perenne de tonos
plateados, muy ornamental. Sus hojas son de textura fina, de color verde y sus flores amarillas que aparecen en racimos grandes. 
Florece de Enero a Marzo (Hemisferio Norte). Legumbre de 5-9 cm de longitud, recta o ligeramente curvada, con los bordes algo 
constreñidos entre las semillas, que se disponen en el fruto longitudinalmente...',100,14,11);
...
/*1. Se quiere saber la segunda palabra del nombre de los clientes y mostrarla en mayúsculas. En caso de que no tenga, no debe 
aparecer en la lista. No deben aparecer espacios ni antes ni después de esta segunda palabra. Se entiende que la segunda palabra
es aquella que está entre el primer espacio y el segundo espacio.*/
SELECT UPPER(TRIM(SUBSTR(NOMBRE_CLIENTE,INSTR(NOMBRE_CLIENTE,' ',1,1),instr(nombre_cliente||' ',' ',1,2)-instr(nombre_cliente,' ')))) FROM CLIENTE
 where upper(trim(substr(nombre_cliente,instr(nombre_cliente,' ',1,1),instr(nombre_cliente||' ',' ',1,2)-instr(nombre_cliente,' ')))) is not null;

/*2. Se quiere mostrar en una única columna y fila una frase que contenga el nombre del cliente que hizo el último pago, el nombre
del cliente que hizo el primer pago y la diferencia en meses (entero inferior, sin decimales) entre ambos pagos. El resultado
debería ser tal que así tras las concatenaciones de dichos tres campos: 'Sotogrande y Tendo Garden. Han pasado 38 meses'. */
select nombre_cliente from pago join cliente on cliente.codigo_cliente = pago.codigo_cliente
    where fecha_pago = (select max(fecha_pago) from pago);
select nombre_cliente from pago join cliente on cliente.codigo_cliente = pago.codigo_cliente
    where fecha_pago = (select min(fecha_pago) from pago);
select floor(months_between((select max(fecha_pago) from pago),(select min(fecha_pago) from pago))) from dual;
select (select nombre_cliente from pago join cliente on cliente.codigo_cliente = pago.codigo_cliente
    where fecha_pago = (select max(fecha_pago) from pago))
    || ' y ' ||
    (select nombre_cliente from pago join cliente on cliente.codigo_cliente = pago.codigo_cliente
    where fecha_pago = (select min(fecha_pago) from pago))
...
SELECT * FROM CLIENTE;
SELECT * FROM PAGO;
SELECT UPPER(CLIENTE.NOMBRE_CLIENTE)"CLIENTE",TO_CHAR(PAGO.TOTAL,'99,999.00L')AS "CUENTA MÁS ALTA" FROM CLIENTE JOIN PAGO ON CLIENTE.CODIGO_CLIENTE = PAGO.CODIGO_CLIENTE;

SELECT UPPER(CLIENTE.NOMBRE_CLIENTE)"CLIENTE",TO_CHAR(PAGO.TOTAL,'99,999.00L')AS "CUENTA" FROM CLIENTE JOIN PAGO ON CLIENTE.CODIGO_CLIENTE = PAGO.CODIGO_CLIENTE
    WHERE PAGO.TOTAL = (SELECT MAX(TOTAL)FROM PAGO) OR PAGO.TOTAL = (SELECT MIN(TOTAL)FROM PAGO);


/*3. Empleando IN, obtén el nombre y gama de todos los productos que sean de la gama Ornamentales, Frutales y Aromáticas, y el 
tamaño/número de caracteres del nombre sea inferior o igual a 10 caracteres. */
select distinct nombre,gama 
    from producto
    where length(nombre) <= 10 and gama in ('Ornamentales','Frutales','Aromáticas');

/* 4. Se quiere saber el código de los pedidos que no tienen comentarios y además la fecha del pedido se encuentra entre enero de 
2008 y febrero de 2009. */
select codigo_pedido from pedido where comentarios is null and fecha_pedido between to_date('2008-01-01') and to_date('2009-02-01');
...
- nombre: 'Luis Miguel'
- localidad: 'Gines' */
insert into clientes values (4,'Luis Miguel',null,'Gines');
...
/*6. Se quiere modificar el nombre de un plato(tabla platos). Cambiar el nombre del plato cuya longitud de caracteres es la
mayor de toda la tabla, añadiendo al final del nombre '- cocina de autor'. Ej.: Entrante Crema de zanahorias - cocina de autor.*/
select * from platos;
...
--Supuesto que queremos hacerlo solo de un registro, el último de la misma hora y día
select max(id) from comandas where fecha = '02/03/2021' and hora = '22:05';
update comandas 
    set hora = '22:05' 
    where id = (select max(id) from comandas where fecha = '02/03/2021' and hora = '22:05');
...
alter table comandas drop constraint SYS_C008668;
...
select initcap(nombre) from ingredientes where categoria = 'PESCADO'; 
...
select distinct estado,mesa from comandas order by estado desc, 2 asc;
select estado from comandas group by estado;
...
select count(*) 
    ...
    where hora between '21:10' and '21:50' and fecha = '01/03/2021' and nombre = 'Pedro Juan';
--subconsulta
select count(*) from comandas where hora between '21:10' and '21:50' and fecha = '01/03/2021' and 
    id_cliente = (select id from clientes where nombre = 'Pedro Juan');
...
select datoscompletos.nombre "CLI", datoscompletos.localidad "LOC" from clientes datoscompletos where localidad = 'Gines';
...
de caracteres. Ej.: Maria: 5*/
select nombre || ': ' || length(nombre) from clientes where length(nombre) = (select min(length(nombre)) from clientes);