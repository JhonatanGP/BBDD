...
SELECT UPPER(REPLACE(SUBSTR(NOMBRE,1,INSTR(NOMBRE||' ',' ')),' ','')) AS NOMBRE FROM CLIENTES WHERE TELEFONO LIKE '%333%';
...
select upper(nombre) from ingredientes where estado = 'LIQUIDO' and substr(nombre,1,1) not in ('A','E','I','O','U');
...
--5. Muestra el nombre de los platos pero con formato distinto. Quita palabras "Plato" y "Entrante", la primera letra en mayúsculas de cada palabra, e 
--incluir un punto y final al terminar de mostrar el nombre del plato. No debe quedar ningún espacio ni al principio ni al final del nombre del plato.
select initcap(trim(replace(replace(nombre,'Plato',''),'Entrante',''))||'.') from platos;
...
select distinct(platos.nombre),hora,fecha from platos join comandas on platos.id = comandas.id_plato where estado = 'SERVIDO' and fecha = '01-03-21' and hora between '21:00'and '21:30';
...
--10. Muestra el nombre, teléfono y localidad de los clientes que viva en Bormujos y su teléfono no empiece por 1. 
--Debes usar alias para nombrar las tres columnas, la primera será "Cliente", la segunda "Tfno" y la tercera "Loc".
...
SELECT NOMBRE ||':' ||LENGTH(NOMBRE) "NOMBRE" FROM INGREDIENTES WHERE LENGTH(NOMBRE) = (SELECT MIN(LENGTH(NOMBRE)) FROM INGREDIENTES);
...
SELECT REPLACE(UPPER(NOMBRE),' ','')"NOMBRE",LOCALIDAD FROM CLIENTES;
...
--22. ¿Cuánto dinero se ha obtenido del plato de ID = 5? Indica solo una columna con el importe con dos decimales. 
--Recuerda que para obtener dinero de un plato, los comensales han debido de pagar la comanda, es decir, tener un estado COBRADO.
SELECT * FROM INGREDIENTES;
SELECT * FROM INGREDIENTES_PLATOS;
SELECT * FROM PLATOS;
SELECT SUM(PRECIO) FROM PLATOS;
...
--30. Indica la CATEGORIA del ingrediente Perejil. Debes mostrar dicha categoría con el tamaño/número de caracteres que sea el tamaño de la categoría que tenga menos letras. En este caso, es OTRO, que tiene 4 caracteres, pero no puedes usar el número 4, debes calcular en tu query ese tamaño mínimo.

--31. Se quiere saber el nombre del cliente, el tiempo de preparación del plato que pidió, el nombre del ingrediente que se usa en mayor cantidad en dicho plato que pidió y la categoría de dicho ingrediente, para aquella comanda realizada en la mesa 1 que fue DEVUELTO.
