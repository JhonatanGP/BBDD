/*# REPASO
Base de datos: FABRICA.SQL */

/*1. Devuelve el nombre del cliente y el precio de venta de la máquina para los casos de línea de fabricación 2, estado = PUESTA A PUNTO y 
fecha año 2023. */
...
select nombre_empresa, precio_venta from CLIENTES 
    join AVANCES_LINEAS_FABRICACION on clientes.cif = AVANCES_LINEAS_FABRICACION.id_cliente 
    join maquinas on maquinas.ref = avances_lineas_fabricacion.id_maquina 
        where avances_lineas_fabricacion.linea_fabricacion = 2 and estado = 'PUESTA A PUNTO' and to_char(fecha,'yyyy') = 2023;

/*2. Se quiere saber cuánta venta se ha hecho (suma total) clasificado por cada nombre de cliente. Se entiende por venta cuando una máquina 
está en los avances de línes de fabricación, independientemente del estado en el que se encuentre.*/
select sum(precio_venta) from maquinas;
select nombre_empresa, sum(precio_venta) from CLIENTES 
    join AVANCES_LINEAS_FABRICACION on clientes.cif = AVANCES_LINEAS_FABRICACION.id_cliente 
    join maquinas on maquinas.ref = avances_lineas_fabricacion.id_maquina group by nombre_empresa;
        
/*3. ¿Cuántas piezas hay por tipo y estado? Muestra una lista que muestre tipo, estado y número de piezas.*/
select * from piezas;
select count(*),tipo,estado from piezas group by tipo,estado;
select count(*)"CANTIDAD",tipo,estado,sum(cantidad)"TOTAL" from piezas join piezas_maquinas on piezas.idpie = piezas_maquinas.id_pieza group by tipo,estado;

/*4. Crea un bloque de código anónimo que pida al usuario un nombre de empresa (cliente) y compruebe, primero si existe o no ese cliente 
en la base de datos. Si no existe, que muestre el mensaje 'No existe ese cliente'. En caso de que exista, devuelve el número total de 
avances de líneas de fabricación asociados a ese cliente mostrando el siguiente mensaje 'Hay ZZZ líneas para el cliente AAA', siendo ZZZ 
el número total de esos avances de ese cliente, y AAA el nombre del cliente.*/
set serveroutput on;
declare
    nombre clientes.nombre_empresa%type := '&nombre_empresa'; -- Pido el nombre de la empresa
    cont int := 0;
    cuentaAvances int := 0;
begin
    select count(*) into cont from clientes where nombre_empresa = nombre;
    if cont = 0 then
        dbms_output.put_line('No existe ese cliente');
    else
        select count(*) into cuentaAvances from avances_lineas_fabricacion join clientes on cif = id_cliente where nombre_empresa = nombre;
        dbms_output.put_line('Hay ' || cuentaavances || ' líneas para el cliente ' || nombre);
    end if;
end;
/

/*5. Crea un bloque de código anónimo que pida al usuario el nombre de un mes (Ej.: AGOSTO). Debes comprobar si en ese mes hay avances de 
líneas de fabricación, y en el caso de que los haya, debes devolver el precio de venta de la máquina más cara que esté en los avances en ese mes.*/
declare
    meteCodigo dept.deptno%type := &mete_codigo;
    contador int := 0;
    contador2 int := 0;
begin
    select count(*) into contador from dept where deptno =meteCodigo;
    if contador = 0 then
        dbms_output.put_line('No existe ese codigo ' || meteCodigo );
    else
        select count(*) into contador2 from emp where deptno = meteCodigo;
        if contador2 = 0 then
            dbms_output.put_line('Existe el código pero no hay empleados en ese departamento');
        else 
            dbms_output.put_line('Hay ' || contador2 || ' empleados en ese departamento');
        end if;
    end if;
end;
/
...
select upper(pais), count(*) from cliente group by pais having count(*) = (select max(count(*)) from cliente group by pais);
...
/*6. Devuelve en una columna el nombre de contacto, en otra el apellido de contacto y en una tercera el número de palabras que hay en el 
apellido del contacto en la tabla cliente (se debe suponer que solo es posible 1 o 2 apellidos a lo sumo, separados por un espacio siempre).*/

/*7. Devuelve el nombre de los cliente junto al caracter : y el campo limite_credito de aquellos clientes cuyo límite de crédito está entre
10000 y 50000. La cadena debe mostrarse en una única columna y terminar con la palabra "euros". */

/*8. Se quiere saber el nombre del empleado y el número de clientes que representa como ventas, ordenados primero por el que tiene más 
clientes representados (de mayor a menor), y luego por el nombre del empleado en orden alfabético (A a Z).*/

--9. ¿Cuántos códigos postales de clientes tienen un 8 o un 9 entre sus cifras?

--10. Nombre de clientes que no tienen línea de dirección 2, su ciudad es Madrid y su línea de dirección 1 acaba en 4.
...
/*10. Muestra el número de pedido, el nombre del cliente, la fecha de entrega y la fecha requerida  de los pedidos que no han sido 
    entregados a tiempo.*/
...
/*23. Mostrar la información del pedido (codigo, fechapedido, fechaesperada, fechaentrega, nombre cliente y total en euros) ordenado por 
total de forma descendente.*/
