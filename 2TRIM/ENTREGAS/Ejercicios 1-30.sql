...
--5. Calcula sueldo neto de cada empleado, la retencion IRPF mensual, sabiendo que la retención es 19% para el sueldo y 10% de la comisión, (redondear a 2 decimales). Si el empleado no tiene comisiÃ³n, se debe mostrar el valor 0.
...
--10. Hacer un listado de empleados (nombre, puesto, sueldo, comision) que tengan asignada comisiÃ³n.
SELECT ENAME,JOB,SAL,COMM FROM EMP WHERE COMM IS NOT NULL;

--11. Hacer un listado de empleados (nombre, puesto, sueldo, comision) que NO tengan asignada comisiÃ³n.
SELECT ENAME,JOB,SAL,COMM FROM EMP WHERE COMM IS NULL;

--12. Seleccionar los empleados cuyo nombre empieza por 'A'.
SELECT ENAME FROM EMP WHERE ENAME LIKE 'A%';

--13. Seleccionar los empleados cuyo nombre tienen una A en cualquier posiciÃ³n.
SELECT ENAME FROM EMP WHERE ENAME LIKE '%A%';
...
--20. Seleccionar los empleados cuyo sueldo es la 5000/2 + 500 ,5000-1000 o el salario mÃ¡ximo de todos los empleados.
SELECT ENAME,SAL FROM EMP WHERE SAL = (5000/2) + 500 OR SAL = 5000-1000 OR SAL =(SELECT MAX(SAL) FROM EMP);

--21. Seleccionar nombre, sueldo y sueldo formateado (ej.: 1,000.00) de todos los empleados. Se puede usar las mÃ¡scaras de to_number en to_char.
SELECT ENAME "NOMBRE",SAL "SALARIO",TO_CHAR(SAL,'9,999.00')"SUELDO FORMATEADO"  FROM EMP;

--22. Seleccionar nombre, sueldo y sueldo formateado con el simbolo 'Dolar' (ej.: $1,000.00) de todos los empleados. Se puede usar las máscaras de to_number en to_char.
SELECT ENAME "NOMBRE",SAL "SALARIO",TO_CHAR(SAL,'9,999.00$')"SUELDO FORMATEADO"  FROM EMP;
...
--25. Seleccionar la fecha del sistema(nomb día, día, nomb mes, año, horas (24):minutos:segundos). No debe haber espacios sobrantes en el nombre del día o del mes.
...
--26. Mostrar la fecha del día "1 de enero de 2005", usando una tira de caracteres y su máscaras de formato (ej.: 01012021), convertirlo en fecha y empleado en el to_char. No deben aparecer espacios adicionales en el nombre del mes.
SELECT TO_CHAR(TO_DATE(10012021,'DD-MM-YYYY'), 'DD "de" MONTH "de" YYYY')"FECHA" FROM DUAL; 
--27. Calcular el número de día vividos hasta hoy por una persona nacida el día 3 de julio de 1970.
SELECT TRUNC(SYSDATE - TO_DATE('03-07-1970','DD,MM,YYYY')) "DÍAS " FROM DUAL;
...
SELECT LOC FROM DEPT JOIN EMP ON EMP.DEPTNO = DEPT.DEPTNO WHERE ENAME = 'KING';
